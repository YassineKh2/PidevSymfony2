...
class FormateurController extends AbstractController
{
    ...
    public function new(Request $request, FormateurRepository $formateurRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $pictureFile = $form->get('ImageFormateur')->getData();
            if ($pictureFile) {
                $pictureFileName = uniqid() . '.' . $pictureFile->guessExtension();
                $pictureFile->move(
                    $this->getParameter('pictures_directory_formateur'),
                    $pictureFileName
                );
                $pictureFileName = 'Front/formateur/images/' . $pictureFileName;
                $formateur->setImageFormateur($pictureFileName);
            }
            else
                $formateur->setImageFormateur("Front/formateur/images/NoImageFound.png");
